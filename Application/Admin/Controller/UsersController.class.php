...
class UsersController extends BaseController
{
    public function url_user(){
        $url_user = realpath('./')."\\Public\\Uploads\\itempic\\";
        return $url_user;
    }
    ...
    {
        //调用模型层获取数据
        $usersmodel = M('u_information');
        $user = $usersmodel->order('i_id desc')->select();
        //调用视图层显示数据
        $this-> users = $user;
        $this->display();
    }
    ...
    public function changestate()
    {
        ...
        if (false != $usersmodel->where("i_id=$i_id")->save()) {
            $tempdata = "修改成功！";
        } else {
            $tempdata = "修改失败！";
        }
    ...
    public function editpswd(){
        $usersmodel=M('user');
        $u_id = I('u_id');
        $usersmodel->password=md5('Lovekn0827');
        if(false != $usersmodel->where("u_id=$u_id")->save()){
            $tempdata = "重置成功！";
        }else{
            $tempdata = "重置失败";
        }
    ...
    public function del_user(){
        $user = M('user');
        $u_id = I('u_id');
        $user_img = M('u_information') ->where("u_id = $u_id")->getField("pic");
        if (false != $user->where("u_id = $u_id")->delete()) {
            if ($user_img != "default.jpg"){
                $user_img_url = $this->url_user().$user_img;
                unlink($user_img_url);
            }
            $tempdata = "删除成功！";
        } else {
    ...
    public function add_user(){
        ...
        for ($i = 0; $i<$k ; $i+=1){
            ...
            if (false != M('user') -> add($user)){
                $u_id = M('user')->where($user)->getField('u_id');
                $message['u_id'] = $u_id;
                ...
                $message['sex'] = rand(0,1);
                $message['age'] = rand(20,50);
                $message['pic'] = rand(1,30).".jpg";
                $message['address'] = $city_arr[array_rand($city_arr)];
                if (false != M('u_information')->add($message)){
                }else{
                    M('user')->where("u_id = $u_id")->delete();
                    $i-=1;
                }
            }
        }
